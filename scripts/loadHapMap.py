...
from simuOpt import *
setOptions(optimized=True, alleleType='lineage')
from simuPOP import *
...
def createHapMapPop(logger=None):
    if logger is not None:
    ...
    pop = Population(size=popSize, ploidy=2, loci=[len(lociPos)],
        ...
        alleleNames=alleleNames, subPopNames=["CEU","YRI"])
    return pop

def loadHapMapPop(pop, chrom, popName, logger=None):
