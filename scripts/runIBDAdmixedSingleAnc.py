...
num_snps = 1000
dir="../data/"
filename="AfricanAmericans8"

h = LDModel(num_snps,1,8,25)
h.set_alphas([1])
h.read_from_bgl_file("/home/eskin/Data/HapMap/HapMap3_CEU_YRI_chr1.bgl.dag",0)
...
h.read_haplos(dir+filename+".genos.dat",200)
...
pairs_f = open(dir+filename+".pairs.dat")
...
out = open(dir+filename+".IBDAdmixedSingle.dat", 'w')
out_ibdprobs = open(dir+filename+".IBDAdmixedSingle.ibdprobs.dat", 'w')
out_no_ibdprobs = open(dir+filename+".IBDAdmixedSingle.noibdprobs.dat", 'w')
num_win = int(num_snps / 25)
for (ind1,ind2) in pairs[0:1]:
    ...
    chr_pair = (0,1,0,1)
    h.calc_top_level_ems_probs(ind1*2+chr_pair[0],ind1*2+chr_pair[1],ind2*2+chr_pair[2],ind2*2+chr_pair[3],dir+"inner.probs.txt")
