#include "GERMLINE.h"
#include "GERMLINE_0001.h"
#include <string.h>

float MIN_MATCH_LEN = 3;
...
{
	return germline_main(argc, argv);
}
...
int germline_main(int argc, char* argv[])
{
	...
	for(int i=1;i<argc;i++){
		...
		if( strncmp(argv[i], "-prefix", strlen("-prefix")) == 0 && i < argc-1)				prefix = argv[++i];
		else if( strncmp(argv[i], "-min_m", strlen("-min_m")) == 0 && i < argc-1)			MIN_MATCH_LEN = atof(argv[++i]);
