...
void MatchesBuilder::matchAllMarkers()
{
	...
	{
		if ( !SILENT ) cout << "\rMatching Markers - " << (position_ms*100) / (ms_end - 1) << "%" << flush;
		matchMarkerSet();
	}
	if ( !SILENT ) cout << '\r' << "Matching Markers Complete" << endl;
}
...
void MatchesBuilder::readAllMarkers()
{
	...
	{
		if ( !SILENT ) cout << "\rReading Markers - " << position_ms*100/ms_end << "%" << flush;
		if ( HAPLOID ) readHaploidMarkerSet(); else readMarkerSet();
	}
	if ( !SILENT ) cout << '\r' << "Reading Markers Complete" << endl;
}
