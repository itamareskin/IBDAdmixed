...
cdef class TestSet(object):
    ...
    cpdef GenotypePair get_genotype_pair(self, int ind1, int ind2):
        ...
        '''
        if ind1 not in range(self._nr_haplos) or ind2 not in range(self._nr_haplos):
            raise ValueError
        cdef TestSet other = GenotypePair()
        other._snp_num = self._snp_num
        other._haplos = <bool **> malloc(other._nr_haplos * sizeof(bool *))
        cdef int hap_idx
        cdef int snp_idx
        for hap_idx in range(other._nr_haplos):
    ...
    cpdef Genotype get_genotype(self, int ind):
        if ind not in range(self._nr_haplos):
            raise ValueError
        cdef TestSet other = Genotype()
        other._snp_num = self._snp_num
        other._haplos = <bool **> malloc(other._nr_haplos * sizeof(bool *))
        cdef int hap_idx
        cdef int snp_idx
...
cdef class Genotype(TestSet):
    ...
    def __getitem__(self, item):
        ...
        res = ''
        for snp_idx in range(self._snp_num):
            if self._haplos[item][snp_idx] == 0:
                res += '0'
            else:
                res += '1'
        return res
    ...
    def cross_hamming_dist(self, Genotype other):
        cdef int snp_idx
        hamming_dist = [0]*4
        for snp_idx in range(self._snp_num):
            for hap_idx1 in range(2):
                for hap_idx2 in range(2):
                    hamming_dist[2*hap_idx1+hap_idx2] += abs(self._haplos[hap_idx1][snp_idx] - other._haplos[hap_idx2][snp_idx])
        return hamming_dist
